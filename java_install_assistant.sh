...
if [ $? -eq 0 ]; then
	...
	if [ $? -lt 11]; then
		echo "$(tput setaf 10)[Btzinho bot]:$(tput setaf 7)  Opa! parece seu java é muito antigo para rodar nossa aplicação, vamos instalar um java mais recente?"
		...
		if [ $VERSAO -eq 11 ]; then
			echo "$(tput setaf 10)[Btzinho bot]:$(tput setaf 7) Preparando para instalar a versão 11 do Java. Confirme a instalação quando solicitado ;D"
